...
void Bill::display_bill()
{
  int tprice=0;
  std::cout<<"|---------------------|"<<endl;
  std::cout<<"     BILLING MENU      "<<endl;
  std::cout<<"|---------------------|"<<endl;
  std::cout<<"Product\t"<<"Price"<<endl;
  for(int i=0;i<pName.size();i++)
  {
    std::cout<<pName[i]<<"\t"<<pprice[i]<<endl;
    tprice+=pprice[i];
  }
  std::cout<<"---------------"<<endl;
  std::cout<<"total\t"<<tprice<<endl;
  std::cout<<"---------------"<<endl;
  pprice.clear();
  pName.clear();
}
...
void Bill::get_details(Bill b)
{
  ...
  do
  {
    ...
    std::cin>>id;
    std::cout<<b.get_product_name(id)<<"\t"<<b.get_product_price(id)<<endl;
   }while(id!=0);
   b.display_bill();
}
